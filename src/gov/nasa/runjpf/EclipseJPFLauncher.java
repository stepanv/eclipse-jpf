...
public class EclipseJPFLauncher extends JPFLauncher {
   ...
   * @return the jpf killer associated with the currently running JPF
   *         or null if the JPF process has already terminated.
   */
   ...
   * @param filepath - the absolute path to the file to be opened
  ...
  /**
   * Returns the arguments for the host VM stored by plugin, configured by the options
   * pane
   * @param def - the value to return if no arguments are stored
   * @return the arguments for the host VM or def if no arguments for the host VM are stored
   */
  @Override
   ...
   * pane
   * @param def - the value to return if no arguments are stored
   * @return the arguments for JPF or def if no arguments for the host VM are stored
   */
  @Override
   ...
   * Identical to <code>getProperty(SITE_PROPERTIES_PATH, DEFAULT_SITE_PROPERTIES_PATH);</code>
  ...
  /**
   * The <code>JPFKiller</code> class is used to help  facilitate canceling a 
   * JPF process
   * @author sandro
   */
  class JPFKiller extends Thread{
    ...
    /**
     * Destroys the JPF thread, removes itself as a ShutdownHook and sets
     * itself to null.
     */
    @Override
    public void run(){
